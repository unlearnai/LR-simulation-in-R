...
# model uses a covariate with random errors
...
lrprd_adj.fun = function(data) {
  N = dim(data)[1]
  m = glm(y ~ as.factor(arm)+c_err, data=data, family='binomial')
...
########################
#Scenario b: covariates with random error, w/ treatment effect
# beta0 = 0
beta1 = 0.75
# beta2 = 1
#Save the results
...
for (i in 1:M) {
  ...
  logit1 = c+beta1    #true logit if assigned to treatment arm
...
save(true_OR, true_RR, true_RD, est_original, OR_unadj_ci, RR_unadj_ci, RD_unadj_ci,
     OR_adj_ci, RR_adj_ci, RD_adj_ci, file = "logistic_trt_randerr.RData")
