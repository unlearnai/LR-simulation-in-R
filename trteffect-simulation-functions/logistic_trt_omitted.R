...
# model omits an important correlated covariate
...
load('/Users/liyunfan/Documents/Simulation_logistic/trteffect-simulation-results/logistic_trt_omitted.RData')

#Number of patients in a dataset
N=800
#Generate M datasets
M=500
#Within each dataset, use this number of bootstrap samples
num_boot=5000
...
#Input: dataset, treatment arm assignment='arm', a MISSPECIFIED SINGLE covariate='c_err'
...
########################
#Scenario 6b: important omitted covariate
# beta0 = 1
beta1 = 0.75
# beta2 = 1
beta3 = 1
...
for (i in 1:M) {
  ...
  logit0 = c+beta3*u    #true logit if assigned to control
  p0 = exp(logit0)/(1+exp(logit0))
  logit1 = c+beta1+beta3*u    #true logit if assigned to treatment
  p1 = exp(logit1)/(1+exp(logit1))
  logit0_omitted = c    #logit if assigned to control, with u omitted
  p0_omitted = exp(logit0_omitted)/(1+exp(logit0_omitted))
  # Generate responses
  y = rbinom(N,1,p)
  ...
  p0_var[i] = var(p0_omitted)
  p0_mean[i] = mean(p0_omitted)
}
...
save(true_OR, true_RR, true_RD, est_original, OR_unadj_ci, RR_unadj_ci, RD_unadj_ci,
     OR_adj_ci, RR_adj_ci, RD_adj_ci, file = "logistic_trt_omitted.RData")
