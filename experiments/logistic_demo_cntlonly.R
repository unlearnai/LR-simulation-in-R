...
# Balanced design, reduction in control arm only

rm(list=ls())
setwd('/Users/liyunfan/Documents/Simulation_logistic')

N=2000
#Scenario 2: With treatment effect; 2:1 randomization ratio
beta0 = 0.5
beta1 = 0.5
beta2 = 2
#Repeat M times
...
for (i in 1:M) {
  ...
  #Take subset of data by taking out control arm, and fit adjusted model
  sub_ind = sample(which(mydata[,'arm']==0), ceiling(N*(2*inf_factor[i]-1)/2), replace=F)
  sub_ind = c(sub_ind,which(mydata[,'arm']==1))
