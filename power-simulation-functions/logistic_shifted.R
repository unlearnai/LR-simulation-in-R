...
for (i in 1:M) {
  ...
  c_err = c+rnorm(N,0,1)+0.5            #covariate with error and systematic bias
  logit0_err = c_err
  p0_err = exp(logit0_err)/(1+exp(logit0_err))        #p0 with random error
  cov_cor[i] = cor(p0, p0_err)        #correlation between p0 and p0_err
  #Generate responses
  y = rbinom(N,1,p)
  mydata = data.frame(arm,c,c_err,y)
  ...
  #Fit adjusted model
  m2 = glm(y ~ as.factor(arm)+c_err, data=mydata, family='binomial')
...
round(cbind(bias_factor, var_factor, eff_factor_adj, E_p0, Var_p0),2)
#Report the std of the above
round(cbind(bias_factor_std, var_factor_std, eff_factor_adj_std, E_p0_std),2)
...
#in all three tests
round(cbind(E_p0, Var_p0, eff_factor_adj, z_ratio, power_m1*100, power_m2*100, z_RD_ratio, power_RD_m1*100, power_RD_m2*100,
            z_lgRR_ratio, power_lgRR_m1*100, power_lgRR_m2*100),2)
#Report the std of the above
round(cbind(E_p0_std, eff_factor_adj_std, z_ratio_std, z_RD_ratio_std, z_lgRR_ratio_std),2)
...
save(list=c('m1_est','m2_est','m1_std','m2_std','m1_z','m2_z',
            ...
            'm1_lgRR_z','m2_lgRR_z'), file='logistic_shifted.rdata')
